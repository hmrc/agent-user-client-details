...
trait JobMonitoringService {
   ...
   */
  def getNextJobToCheck(using ec: ExecutionContext): Future[Option[WorkItem[JobData]]]
  def createFriendlyNameFetchJobData(jobData: FriendlyNameJobData)(using ec: ExecutionContext): Future[ObjectId]
  def markAsFinished(objectId: ObjectId)(using ec: ExecutionContext): Future[Unit]
  def markAsNotFinished(objectId: ObjectId)(using ec: ExecutionContext): Future[Unit]
...
extends JobMonitoringService {
  ...
  def getNextJobToCheck(using ec: ExecutionContext): Future[Option[WorkItem[JobData]]] = jobMonitoringRepository.pullOutstanding(
  ...
  )(using ec: ExecutionContext): Future[ObjectId] = jobMonitoringRepository
    .pushNew(jobData)
    .map(_.id)

  def markAsFinished(objectId: ObjectId)(using ec: ExecutionContext): Future[Unit] = jobMonitoringRepository.markAs(objectId, Succeeded).map(_ => ())

  def markAsNotFinished(objectId: ObjectId)(using ec: ExecutionContext): Future[Unit] = jobMonitoringRepository.markAs(objectId, Failed).map(_ => ())
